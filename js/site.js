...
(() => {
  ...
  if (panel && dataEl) {
    ...
    const card = i => `<article class="product${i.sold ? '' : ''}" data-artist="${i.as}" data-avail="${i.sold ? 'sold' : 'available'}"><a class="frame" href="${i.url}"><img src="${i.img}" alt="${i.t}" loading="lazy">${i.sold ? '<span class="pill sold">Sold</span>' : ''}</a><p class="a">${i.a}</p><p class="t"><i>${i.t}</i>${i.y ? ', ' + i.y : ''}</p><p class="p${i.sold ? ' sold' : ''}">${i.sold ? 'Sold' : i.p}</p><div class="buy">${i.sold ? '<span class="btn is-sold">Sold</span>' : `<a class="btn btn-accent" href="${i.buy}" target="_blank" rel="noopener">${i.direct ? 'Buy now' : 'Add to cart ↗'}</a>`}<a class="btn btn-ghost" href="${i.url}">Details</a></div></article>`;
